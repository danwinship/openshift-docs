...
+
If your cluster requires different MTU values for different nodes, then you must set this value to `50` less than the lowest MTU value in your cluster. For example, if some nodes in your cluster have an MTU of `9001`, and some have an MTU of `1500`, then you must set this value to `1450`.
...
+
If the auto-detected value is not what you expected it to be, confirm that the MTU that is set on your nodes is correct. You cannot use this value to change the MTU value on the underlying nodes.
+
If your cluster requires different MTU values for different nodes, then you must set this value to `100` less than the lowest MTU value in your cluster. For example, if the smallest MTU in your cluster is `900`, then you must set this value to `800`.
endif::operator[]
