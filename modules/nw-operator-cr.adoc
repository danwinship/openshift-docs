...
<4> The maximum transmission unit (MTU) for the VXLAN overlay network. This is
detected automatically, and you should not normally need to override it; if the
auto-detected value is not what you expected it to be, it is generally because
the nodes themselves have the wrong MTU on their primary network interfaces, in
which case manually overriding this option will not fix things. The only time
you need to manually set this option is if different nodes in your cluster have
different interface MTUs (in which case, this option must be set to 50 less than
the lowest node MTU in the cluster).
...
overlay network. This is detected automatically, and you should not normally
need to override it; if the auto-detected value is not what you expected it to
be, it is generally because the nodes themselves have the wrong MTU on their
primary network interfaces, in which case manually overriding this option will
not fix things. The only time you need to manually set this option is if
different nodes in your cluster have different interface MTUs (in which case,
this option must be set to 100 less than the lowest node MTU in the cluster).
endif::operator[]
